...
disp("computing unconstrained transfer cost")
tic
...
scatter(mjd_opt_dep_grid, mjd_opt_arr_grid, "filled", "o", "r");
legend("", "minimum delta-v trajectory point"); hold off;


tof_days_opt_grid = mjd_opt_arr_grid - mjd_opt_dep_grid;

% results output
fprintf("\n=== UNCONSTRAINED GRID SEARCH RESULTS ===\n");
...
tof_days_constrained = NaN(length(depart_times), length(arrive_times));

disp("computing constrained transfer cost")
tic
for i = 1:length(depart_times)
    for j = 1:length(arrive_times)
...
% results output
fprintf("\n=== CONSTRAINED GRID SEARCH RESULTS ===\n");
fprintf("Min Delta V: %.4f km s^-1\n", min_val_constrained);
...
tspan = linspace(t1, t2, orbit_steps);
...
tspan_unused = linspace(t2, t2 + T_unused - t1, orbit_steps); % integration time span array
...
tspan_earth = linspace(t1, t2, orbit_steps); % integration time span array
...
tspan_mars = linspace(t1, t2, orbit_steps); % integration time span array
...
% c. earth orbit
plot3(R_earth(:, 1), R_earth(:, 2), R_earth(:, 3), "g"); % during transfer
%plot3(R_earth(:, 1), R_earth(:, 2), R_earth(:, 3), "r--"); % full orbit

% d. mars orbit
plot3(R_mars(:, 1), R_mars(:, 2), R_mars(:, 3), "r"); % during transfer
