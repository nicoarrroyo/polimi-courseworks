...
% --- Preliminary Disturbances Evaluation ---
% Gravity Gradient
T_gg_prelim = 3*mu / (2*R_orb^3) * abs(I_max - I_min);

% Solar Radiation Pressure (gemini gave 4.5*10e-6)
T_srp_prelim = (F_sun/c_light) * S * (1+q_refl) * (csp_shift);
...
Cd = 2.6; % from fauste
T_aero_prelim = 0.5 * rho * V^2 * A_drag * Cd * cp_shift;
...
theta_gg = 32.5 * (pi/180);   % Off-nadir angle [rad]
...
T_srp = (F_sun / c_light) * S * (1 + q_refl) * cos(i_sun) * csp_shift;
